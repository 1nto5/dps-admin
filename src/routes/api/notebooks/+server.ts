...
export const POST: RequestHandler = async ({ request }) => {
	...
	const dupName = db.select().from(notebooks).where(eq(notebooks.name, trimmedName)).get();
	if (dupName) return json({ error: `Notebook '${trimmedName}' already exists` }, { status: 400 });

	if (trimmedInv) {
		const dupInv = db.select().from(notebooks).where(eq(notebooks.inventoryNumber, trimmedInv)).get();
		if (dupInv) return json({ error: `Inventory number '${trimmedInv}' already exists` }, { status: 400 });
	}

	if (trimmedSerial) {
		const dupSerial = db.select().from(notebooks).where(eq(notebooks.serialNumber, trimmedSerial)).get();
