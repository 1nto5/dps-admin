...
export const load: PageServerLoad = async ({ params }) => {
	...
	return { computer: item, rooms: db.select().from(rooms).orderBy(asc(rooms.name)).all(), users: db.select().from(users).orderBy(asc(users.name)).all(), monitors: assignedMonitors };
};
